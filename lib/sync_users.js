...
function updateUser(mysqlDb, id, user) {
  ...
  mysqlDb.query("UPDATE user_account SET ? WHERE user_id = ?", [dbUser, id], (error, results, fields) => {
    if (error !== null) console.error(error);
  });
}

function deleteUser(mysqlDb, id) {
  mysqlDb.query("DELETE FROM user_account WHERE user_id = ?", id, (error, results, fields) => {
    if (error !== null) console.error(error);
  });
}
